...
> and are used to ***emulate*** a string.
>
> If in doubt,
> ```ts
> mediq().toString();
...
```
```ts
mediq().max.height(50).vh.and.min.width(100).px // @media (min-height: 50vh) and (min-width: 100px)
```
...
```
```ts
mediq().max.height(50).vh.or.min.width(100).px // @media (min-height: 50vh), (min-width: 100px)
```
